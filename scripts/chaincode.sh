...
__init_go_mod() {
    ...
    cd ${CHAINCODE_PATH}/${chaincode_name} >/dev/null 2>&1 || {
        log >&2 "${CHAINCODE_PATH}/${chaincode_name} path does not exist" error
        exit 1
    }
...
chaincode_zip() {
    ...
    mkdir -p ./vendor/${CHAINCODE_REMOTE_PATH}/${chaincode_name} && rsync -ar --exclude='vendor' --exclude='META-INF' . ./vendor/${CHAINCODE_REMOTE_PATH}/${chaincode_name} || {
        log >&2 "Error copying chaincode into vendor directory." error
        exit 1
    }

    zip -rq ${DIST_PATH}/${chaincode_name}_${timestamp}.zip . || {
