...
start_explorer() {
    ...
    cat ${config}.base.json | __jq -r --arg private_key "$private_key" '.organizations.Org1MSP.adminPrivateKey.path = $private_key' |
        __jq -r --argjson FABKIT_TLS_ENABLED "$FABKIT_TLS_ENABLED" '.client.tlsEnable = $FABKIT_TLS_ENABLED' >${config}.json 1>/dev/null 2> >(__throw >&2)
    ...
    (loginfo "Launching containers" && docker-compose --env-file ${FABKIT_ROOT}/.env -f ${FABKIT_EXPLORER_PATH}/docker-compose.yaml up --force-recreate -d 1>/dev/null 2> >(__throw >&2)) &
    __spinner
...
stop_explorer() {
    ...
    docker-compose --env-file ${FABKIT_ROOT}/.env -f ${FABKIT_EXPLORER_PATH}/docker-compose.yaml down 1>/dev/null 2> >(__throw >&2)
